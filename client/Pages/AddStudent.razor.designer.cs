...
namespace CaApp.Pages
{
    public partial class AddStudentComponent : ComponentBase
    {
        ...
        IEnumerable<CaApp.Models.ConData.SchoolClass> _getSchoolClassesResult;
        protected IEnumerable<CaApp.Models.ConData.SchoolClass> getSchoolClassesResult
        {
            get
            {
                return _getSchoolClassesResult;
            }
            set
            {
                if (!object.Equals(_getSchoolClassesResult, value))
                {
                    var args = new PropertyChangedEventArgs(){ Name = "getSchoolClassesResult", NewValue = value, OldValue = _getSchoolClassesResult };
                    _getSchoolClassesResult = value;
                    OnPropertyChanged(args);
                    Reload();
                }
            }
        ...
        protected async System.Threading.Tasks.Task Load()
        {
            ...
            var conDataGetSchoolClassesResult = await ConData.GetSchoolClasses();
            getSchoolClassesResult = conDataGetSchoolClassesResult.Value.AsODataEnumerable();
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(CaApp.Models.ConData.Student args)
        {
            try
            {
                var conDataCreateStudentResult = await ConData.CreateStudent(student:student);
